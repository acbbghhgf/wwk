...
int wtk_stack_write_nbfd(wtk_stack_t *s, int fd) {
	stack_block_t *b;
	int n, writed;
    int ret;

	for(b = s->pop; b ; s->pop = b->next, b->next = 0, b = s->pop) {
		n = b->push - b->pop;
		if (n > 0) {
            ret = wtk_fd_write_nonblock(fd, (char*)b->pop, n, &writed);
            b->pop += writed;
            s->len -= writed;
            if (WTK_EOF == ret || WTK_ERR == ret) {
                ret = -1;
                goto end;
            }
            if (writed != n) {
