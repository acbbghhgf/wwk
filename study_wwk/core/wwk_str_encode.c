/*************************************************************************
	> File Name: wwk_str_encode.c
	> Author: 
	> Mail: 
	> Created Time: Tue 12 Nov 2019 06:21:45 PM PST
 ************************************************************************/
 ...
 */
int wwk_str_convert(const char* src,int src_bytes,const char* src_code,const char *dst_code,char *dst,int dst_bytes)
{
	iconv_t id;
	size_t in,out,c;
	int ret=-1;
 ...
 */
char* wwk_str_convert2(const char* src,int len,const char* src_code,const char *dst_code)
{
	int out,ret;
	char *dst;

	out=len*3+1;
	dst=(char*)wwk_calloc(1,out);
	ret=wwk_str_convert(src,len,src_code,dst_code,dst,out);
	if(ret<0)
	{
		wwk_free(dst);
		dst=0;
 ...
 */
char* wwk_str_convert3(const char* src,int len,const char* src_code,const char *dst_code)
{
#define N 1024
	static char buf[N];
	char *p;
	...
	p=buf;
	memset(buf,0,N);
	ret=wwk_str_convert(src,len,src_code,dst_code,p,N);
	p=ret<0?0:buf;
	return p;
}
...
char* utf8_to_gbk_2(const char* utf,int len)
{
	return wwk_str_convert2(utf,len,"gb18030","utf-8");
}
...
char* gbk_to_utf8_2(const char* gbk,int len)
{
	return wwk_str_convert2(gbk,len,"utf-8","gb18030");
}

char* gbk_to_utf8_3(const char* gbk,int len)
{
	return wwk_str_convert3(gbk,len,"utf-8","gb18030");
}

char* utf8_to_gbk_3(const char *utf8,int len)
{
	return wwk_str_convert3(utf8,len,"gb18030","utf-8");
}
#endif
#endif
